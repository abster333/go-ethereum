...
description: Perform a Geth (Go Ethereum) security review and exploitable bug hunt, prioritizing bug-bounty context.
---------------------------------------------------------------------------------------------------------
...
## CRITICAL CONTEXT & SCOPE REDUCTION

**BEFORE STARTING ANY ANALYSIS**, you MUST review the contents of the `bug-bounty/` directory. This folder contains crucial context about the project, attack surface, and **already tested vectors**.

1.  **Review `bug-bounty/01-PROJECT-OVERVIEW.md` & `02-ATTACK-SURFACE.md`**: Understand the high-level architecture and prioritized attack surfaces.
2.  **Review `bug-bounty/TestedVectors.md`**: This file lists vectors that have already been analyzed. **DO NOT** waste time re-analyzing these unless you find a novel bypass.
3.  **Review `bug-bounty/report.md`**: This contains details of a recently found High-Severity CPU DoS. Understand this to avoid reporting duplicates.
4.  **Review `bug-bounty/06-HISTORICAL-ISSUES.md`**: Understand past vulnerability patterns to guide your search for similar issues.
...
### Phase 0 – Context & Scope Definition

* **MANDATORY**: Read the files in `bug-bounty/` to build your mental model.
* **CHECK**: Cross-reference any potential finding with `bug-bounty/TestedVectors.md` and `bug-bounty/report.md` to ensure it is not a duplicate or already tested vector.
...
Begin your analysis in four steps:

1. **Context Loading**
   Read `bug-bounty/` files to understand the project, attack surface, and exclusions.
...
4. **False-Positive Filtering & Reporting**
