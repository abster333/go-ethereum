...
1.  **Transport Layer (`p2p/rlpx/rlpx.go`):**
    The `Conn.Read()` method reads messages. The `eth` protocol enforces a `maxMessageSize` of 10MB (defined in `eth/protocols/eth/protocol.go`, enforced in `eth/protocols/eth/handler.go`), which is the relevant limit for this attack vector.
...
*   **Amplification:**
    ...
    *   **Throttling Factor:** The `TxFetcher` sleeps for 200ms if the rejection rate is high. This limits the processing rate per connection.
    *   **Throughput:** The handler blocks for Processing + Sleep $\approx 197.5\text{ms} + 200\text{ms} \approx 400\text{ms}$.
    *   **Max Rate:** $\approx 2.5$ messages per second per connection.
    *   **CPU Load:** $2.5 \times 197.5\text{ms} \approx 0.5\text{s}$ of CPU work per second per connection.
    *   This means a single attacker connection can saturate **0.5 CPU cores**.
    *   To saturate a standard 8-core node, an attacker needs only **~16 concurrent connections**, which is trivial to establish.
...
7.  **Result:** Victim CPU spikes. Peer is **not disconnected** because `otherreject` only triggers a 200ms sleep. While this throttles the individual connection to ~0.5 cores of load, it does not prevent the attack when scaled across multiple connections.
